...
export function useFolderSnippets(
  titleQuery: string,
  selectedLanguage: string,
  folderData: Folder
) {
  // split refs
  const itemsWithIdx = React.useMemo(
    () => folderData.items.map((snip, idx) => ({ ...snip, idx })),
    [folderData.items]
  );
  const remoteRefs = React.useMemo(
    () => itemsWithIdx.filter((i) => i.kind === 'remote'),
    [itemsWithIdx]
  );
  const local = React.useMemo(
    () => itemsWithIdx.filter((i) => i.kind === 'local'),
    [itemsWithIdx]
  );
  ...
  const fetchRemote = React.useCallback(async () => {
    ...
    const res = await axiosInstance.get<Omit<RemoteSnippet, 'kind'>[]>(
      `/snippets/batch?ids=${ids}`
    );
  ...
  const matches = React.useCallback(
    ({ title, language }: { title: string; language: string }) => {
      const okTitle =
        !titleQuery || title.toLowerCase().includes(titleQuery.toLowerCase());
      const okLang =
